...
import { Activity, DownloadCloud, FileText, Fingerprint, Gauge, Languages, MonitorSmartphone, Search, Settings, ShieldCheck, Users } from 'lucide-react';
    ...
    <nav aria-label="Administration" className="mb-7 overflow-x-auto rounded-2xl border border-cyan-300/10 bg-slate-950/60 p-2 backdrop-blur-xl">
      <div className="flex min-w-max items-center gap-1">
        ...
        {items.map(({ label, href, icon: Icon }) => {
          ...
          return <Link key={label} href={href} aria-current={selected ? 'page' : undefined} className={`flex min-h-11 items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition ${selected ? 'border border-cyan-300/20 bg-cyan-300/10 text-cyan-100' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}><Icon size={16}/>{label}</Link>;
        })}
      </div>
    </nav>
  );
