...
import { CheckCircle2, Clock3, PackagePlus, ShieldCheck, UploadCloud } from 'lucide-react';
...
type Props = { apps: AppRow[]; audit: AuditRow[]; canApprove: boolean; role: string };

export default function AppCenterDashboard({ apps, audit, canApprove, role }: Props) {
  const form = useForm({
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="border-b border-cyan-300/10 bg-slate-950/95 px-5 py-5">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div><p className="text-xs font-bold tracking-[.28em] text-cyan-300">NEXVARY APP CENTER</p><h1 className="mt-1 text-2xl font-black">Publishing Command Center</h1></div>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 p-5 lg:grid-cols-[1.05fr_.95fr]">
        <section className="rounded-3xl border border-white/10 bg-white/[.03] p-5">
          <div className="mb-5 flex items-center gap-3"><PackagePlus className="text-cyan-300"/><div><h2 className="font-black">New application release</h2><p className="text-sm text-slate-400">Assistants prepare drafts. Admin/Owner approves public publishing.</p></div></div>
          <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
            ...
            <input className="rounded-xl border border-white/10 bg-slate-900 p-3" placeholder="Version" value={form.data.version} onChange={e=>form.setData('version',e.target.value)}/>
            ...
            <input className="rounded-xl border border-white/10 bg-slate-900 p-3 md:col-span-2" placeholder="Download URL (APK / EXE / MSI / ZIP)" value={form.data.apk_url} onChange={e=>form.setData('apk_url',e.target.value)}/>
            <input className="rounded-xl border border-white/10 bg-slate-900 p-3" placeholder="SHA-256" value={form.data.sha256} onChange={e=>form.setData('sha256',e.target.value)}/>
            ...
            <button disabled={form.processing} className="md:col-span-2 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-cyan-300/30 bg-cyan-300/10 font-bold text-cyan-100"><UploadCloud size={18}/>Save draft</button>
          </form>
        </section>

        <section className="space-y-6">
          <div className="rounded-3xl border border-white/10 bg-white/[.03] p-5">
            <h2 className="mb-4 font-black">Release queue</h2>
